...
OPEN_HOUR, CLOSE_HOUR, FREQ_MIN, INCLUDE_CLOSE = 8, 18, 30, False
...
def simulate_one_day(target_date, total_pat, weather_text, efficiency_rate=1.0):
    ...
    a, b, alpha, floor_ratio = float(calib.get("a",1)), float(calib.get("b",0)), float(calib.get("alpha",0.3)), float(calib.get("floor_ratio",0.9))
...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        eff = st.slider("処理効率 (スタッフ体制)", 50, 120, 100, 5, help="100%が標準。混雑時や欠員時は下げてください")
