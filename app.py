...
def get_default_weather_metrics(month, weather_text):
    ...
    if "雨" in weather_text: rain, hum, temp = 5.0, 85.0, temp-2.0
    elif "雪" in weather_text: rain, hum, temp = 2.0, 80.0, max(temp-5.0, 0.0)
    elif "晴" in weather_text: hum, temp = 50.0, temp+2.0
    return {"平均気温":temp, "最高気温":temp+5, "最低気温":temp-5, "降水量":rain, "平均湿度":hum, "平均風速":wind}
...
def simulate_one_day(target_date, total_pat, weather_text):
    ...
    for ts in generate_slots(target_date):
        ...
        wait_final = 0.0 if q_start < 0.5 else wait_val
